...
class HelloGodsSpec extends GraphSpec{
  "Dao" should "properly store & read simple classes" in {
  ...
  "Dao" should "properly delete classes" in {
  ...
  "Dao" should "properly associate edges with vertices" in {
  ...
  "Dao" should "properly handle gremlin queries" in {
    ...
    abstractGods.find { _.name == "Jialong" } should be(None)
  }
}
